...
This is a sample template for python-fibo-iter - Below is a brief explanation of what we have generated for you:
...
```bash
sam logs -n HelloWorldFunction --stack-name python-fibo-iter --tail
```
...
```bash
aws cloudformation delete-stack --stack-name python-fibo-iter
```
